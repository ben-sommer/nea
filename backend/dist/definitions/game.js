...
class Game {
    ...
    get scores() {
        const addReduce = (array, condition) => array.reduce((previous, current) => previous + (condition(current) || 0), 0);
        return {
            black: addReduce(this.board.map((line) => addReduce(line, (item) => (item == "black" ? 1 : 0))), (item) => item),
            white: addReduce(this.board.map((line) => addReduce(line, (item) => (item == "white" ? 1 : 0))), (item) => item),
        };
    }
    get otherPlayer() {
        if (this.turn == "black") {
    ...
    checkMove(x, y) {
        ...
        const ray = (x, y, xDir, yDir, squares) => {
